<template>
  <div>
    <div class="cards">
      ...
      <el-card class="card" @click.native="loadMenu1('Network')">
        <div class="top">
          <span class="lfont">menu</span>
          <h1 class="tfont">Network</h1>
        </div>
      </el-card>
    ...
    <el-card
      class="cardMain ps"
      :style="{ height: clientHeight + 'px', color: color }"
      v-show="currentmenu == 'Network'"
    >
      <p v-for="(item, index) in dtseq1" :key="index">{{ item }}</p>
    </el-card>
...
export default {
  ...
  methods: {
    getNetwork() {
      ...
      //     //后台请求数据
      //     this.dtseq1.unshift(res.data.data.value);
      //     this.dtseq1 =
      //       res.data.data.number && this.dtseq1.slice(0, res.data.data.number);
      //     this.color = res.data.data.color && res.data.data.color;
      //     this.await = res.data.data.await && res.data.data.await; //需要开始定义
      //   })
      //   .catch((error) => {
      //     console.log(error);
      //   });
      setTimeout(() => {
        console.log("this.dtseq1===", this.dtseq1);
        this.dtseq1.unshift("999999");
        this.dtseq1 = this.dtseq1.slice(0, 15);
        this.color = "red";
        this.await = "5000"; //需要开始定义
      }, 100);
    },
    //按钮命令
    cmdsend(opName) {
